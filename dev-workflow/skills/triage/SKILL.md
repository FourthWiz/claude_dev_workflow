...
| `/critic` | critic, critique, review the plan, find issues, gaps, risks, architecture | "critique this plan", "review the plan", "find issues with this plan", "what's wrong with this", "check the plan", "critique the architecture", "critique architecture", "review architecture", "find issues with the architecture" | Plan found (+1), no critic-response yet (+1), `architecture.md` exists + no `architecture-critic-*.md` found (+1) | Recent commits exist + plan + no review (−2 — prefer `/review`) | Runs before `/implement` to catch plan gaps. Also accepts `--target=architecture.md` to critique `architecture.md` directly. |
...
| "architecture", "critique arch" | `/architect`, `/critic` | `architecture.md` exists + no plan + prompt contains 'critique'/'review'/'check' → propose `/critic --target=architecture.md` +2, suppress `/architect` −1. `architecture.md` missing → propose `/architect` +2, suppress `/critic` −1. |
