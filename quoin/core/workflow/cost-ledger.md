...
The one-shot historical backfill pass is `quoin/scripts/backfill_cost_attribution.py` — it walks NON-finalized ledgers and appends col 8 to rows that are missing it (recover-or-label only: an unpriceable or ambiguous session is always labeled `unresolved`, never a fabricated cost). Finalized ledgers are skipped by default (kept immutable); `--include-finalized` writes a non-mutating `cost-backfill.json` side-car next to the finalized ledger instead of touching it.
