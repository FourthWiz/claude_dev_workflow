...
- `precompact.sh` fires on the `PreCompact` hook event, but only for **auto** triggers — a
  ...
  avoid creating an orphaned duplicate `[src: quoin/hooks/precompact.sh:261-262]`. Otherwise the
  checkpoint write is gated on a three-row truth table (`IVG-258 S-3`), branched after the
  write `[src: quoin/hooks/precompact.sh:472]`:
  ...
  | 3 | neither (plain conversation) | not written | not written; `QUOIN_PRECOMPACT_NORUN_CHECKPOINT=1` restores checkpoint + sentinel `[src: quoin/hooks/precompact.sh:476-481]` |

  The row-3 sentinel (knob on) points at the checkpoint the same invocation wrote
  `[src: quoin/hooks/precompact.sh:478]`; rows 1 and 2 never write this family.
