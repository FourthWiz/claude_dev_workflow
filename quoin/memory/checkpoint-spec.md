...
1. **`/checkpoint` itself** — writes files named
   ...
   `[src: quoin/hooks/precompact.sh:304]`, or a panic-mode skeleton using the same
...
- `precompact.sh` fires on the `PreCompact` hook event, but only for **auto** triggers — a
  ...
  avoid creating an orphaned duplicate `[src: quoin/hooks/precompact.sh:211-212]`. Otherwise the
  checkpoint write is gated on a three-row truth table (`IVG-258 S-3`), branched after the
  write `[src: quoin/hooks/precompact.sh:422]`:
  ...
  | 3 | neither (plain conversation) | not written | not written; `QUOIN_PRECOMPACT_NORUN_CHECKPOINT=1` restores checkpoint + sentinel `[src: quoin/hooks/precompact.sh:426-431]` |

  The row-3 sentinel (knob on) points at the checkpoint the same invocation wrote
  `[src: quoin/hooks/precompact.sh:428]`; rows 1 and 2 never write this family.
