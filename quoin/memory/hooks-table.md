...
- `QUOIN_RUN_STATE_STALE_DAYS` — one knob, two independent windows, each with its own default:
  ...
  - Probe window, default 14 — read only by `run_state_probe` (`_lib.sh`); its four consumers are the two `userpromptsubmit.sh` advisory branches, `/run`'s 90% self-checkpoint block, and `/checkpoint` Step 1.4's compact-already-ran skip check (not itself a self-checkpoint block — it consumes the probe's answer in the opposite direction from `/run`'s: "no active run" there means fall back to a real save, not skip one). Wider on purpose: the probe has no downstream max-age gate, so its window is the final word on "active or not."
