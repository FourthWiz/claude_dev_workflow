...
**Small — unchanged single-pass review.** Fires when `Task profile: Small`, OR a `Review shape: single-pass (fast-path)` override is present AND `Task profile:` is Small or Medium (never Large — see the carve-out below). Run the entire Review process below exactly as today: zero extra subagents, one `review-N.md` write. This branch is byte-path-identical to the pre-fan-out single-pass flow for a genuine Small task; the review-shape-override case on Small/Medium shares this exact same code path.
