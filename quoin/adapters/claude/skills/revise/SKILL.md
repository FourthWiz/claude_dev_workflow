...
Dispatch action (when pollution detected AND no sentinel AND no prior §0 dispatch):
  ...
  Otherwise spawn an Agent subagent:
    model: "opus"
    description: "revise — pollution-isolated dispatch"
...
Fail-OPEN path:
  ...
  - 1M-credit-class: if the error text contains the substring
      ...
      Issue an `AskUserQuestion`:
        Question: "§0' opus dispatch failed with a 1M-context credit mismatch for /revise.
        ...
        Option 1:
          ...
          standard-context model (e.g., /model opus), then re-invoke /revise.
          The §0' dispatch will then land on standard Opus successfully."
        Option 2:
          label: "Proceed in-session at parent tier"
          description: "Skip the §0' dispatch this once. /revise runs in the
      ...
      switch with /model and re-invoke /revise]` and STOP. Do NOT proceed to skill body.
  ...
  - Any other error (non-1M): Issue an `AskUserQuestion` (generic wording):
      Question: "§0' pollution dispatch failed for /revise. Would you like to proceed
      in the current (polluted) session, or abort?"
      Header: "Dispatch error"
      multiSelect: false
      Option 1:
        label: "Abort — I'll diagnose and retry"
        description: "Stop here. Investigate the dispatch error, then re-invoke /revise."
      Option 2:
...
On fire (happy path — silent up-dispatch):
  spawn an Agent subagent:
    model: "opus"
    description: "revise — min-tier up-dispatch"
...
Fail-OPEN path (fires only when Agent dispatch fails):
  ...
  - 1M-credit-class: if error text contains `Usage credits required for 1M context`:
      Issue AskUserQuestion:
        Question: "§0″ up-dispatch to opus failed with a 1M-context credit mismatch for /revise.
        ...
        Option 1:
          ...
          model (e.g., /model opus), then re-invoke /revise."
        Option 2:
          label: "Proceed in-session at parent tier"
          description: "Skip the up-dispatch this once. /revise runs in the current session
      ...
      switch with /model and re-invoke /revise]` and STOP.
  ...
  - Any other error: Issue AskUserQuestion (labels verbatim — drift relies on equality):
      Question: "/revise requires Opus but this session is below Opus. Auto-dispatch to Opus failed. How would you like to proceed?"
      Header: "Min-tier"
      multiSelect: false
      Option 1:
        label: "Abort — run from an Opus session"
        description: "Stop here. Switch the session to Opus (/model opus) and re-invoke /revise."
      Option 2:
        label: "Proceed at current tier (under-powered)"
        description: "Run /revise on the current cheaper model. Quality may be reduced;
        emits a one-line advisory."
    Then:
      - Option 1: print `[quoin-mintier: aborted; re-invoke /revise from an Opus session]` and STOP.
