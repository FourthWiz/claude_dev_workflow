  ...
  - 1M-context precheck (parent-credit-mismatch guardrail):
    ...
    form AND current_tier > declared_tier:
      ...
      verbatim — drift detection relies on string equality):
        ...
        Question: "Parent session is on a 1M-context model. Dispatching /rollback to sonnet
        ...
        Option 1:
          ...
          to a standard-context model (e.g., /model sonnet), then re-invoke /rollback.
          The §0 dispatch will then land on standard Sonnet successfully."
        Option 2:
          label: "Proceed in-session at parent tier"
          description: "Skip the §0 dispatch this once. /rollback runs on the parent model
      ...
      Then:
        - On Option 1 ("Abort — I'll switch with /model first"): print the single line
          ...
          and re-invoke /rollback]` and STOP. Do NOT proceed to §1. Do NOT spawn any
