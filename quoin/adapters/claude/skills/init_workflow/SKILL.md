  ...
  - 1M-context precheck (parent-credit-mismatch guardrail for §0' opus dispatch):
    ...
    the prompt does NOT start with any `[no-redispatch]` form AND no prior §0 dispatch fired:
      ...
      verbatim — drift detection relies on string equality):
        ...
        Question: "Parent session is on a 1M-context model. Dispatching /init_workflow to opus
        ...
        Option 1:
          ...
          to a standard-context model (e.g., /model opus), then re-invoke /init_workflow.
          The §0' dispatch will then land on standard Opus successfully."
        Option 2:
          label: "Proceed in-session at parent tier"
          description: "Skip the §0' dispatch this once. /init_workflow runs on the parent model
      ...
      Then:
        - On Option 1 ("Abort — I'll switch with /model first"): print the single line
          ...
          and re-invoke /init_workflow]` and STOP. Do NOT proceed to skill body. Do NOT spawn any
...
On fire:
  Issue AskUserQuestion (labels verbatim — drift relies on equality):
    Question: "/init_workflow requires Opus but this session is below Opus. How would you like to proceed?"
    Header: "Min-tier"
    multiSelect: false
    Option 1:
