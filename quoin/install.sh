...
# Phase 8 migration: review installs from the Claude adapter path.
ADAPTER_REVIEW_SRC="$SCRIPT_DIR/adapters/claude/skills/review/SKILL.md"
if [ ! -f "$ADAPTER_REVIEW_SRC" ]; then
  error "Expected Claude adapter SKILL.md at $ADAPTER_REVIEW_SRC but not found — aborting"
  exit 1
fi
...
# Phase 11 migration: gate installs from the Claude adapter path.
ADAPTER_GATE_SRC="$SCRIPT_DIR/adapters/claude/skills/gate/SKILL.md"
if [ ! -f "$ADAPTER_GATE_SRC" ]; then
  error "Expected Claude adapter SKILL.md at $ADAPTER_GATE_SRC but not found — aborting"
  exit 1
fi
...
# Phase 13 migration: rollback installs from the Claude adapter path.
ADAPTER_ROLLBACK_SRC="$SCRIPT_DIR/adapters/claude/skills/rollback/SKILL.md"
if [ ! -f "$ADAPTER_ROLLBACK_SRC" ]; then
  error "Expected Claude adapter SKILL.md at $ADAPTER_ROLLBACK_SRC but not found — aborting"
  exit 1
fi
