"""IVG-71 T-07: install.sh deploys affected_tests.py to both targets.

Two-tier test (modelled verbatim on test_install_branch_hygiene_deployed.py):

1. Non-skipped unit-level assertion (runs on CI without `claude`):
   Import installer.py and assert "affected_tests.py" is in BOTH
   ...
   and 2026-06-08 (a wrapped script missing from either list fails at runtime).

2. Deployment test (dev-machine only — skipif claude/npx absent):
   Run install.sh and assert BOTH deployment targets are present:
     - wrapper at ~/.claude/scripts/affected_tests.py
     - core impl at ~/.claude/core/scripts/affected_tests.py
   Also asserts that the deployed wrapper can be invoked (python3 --help
   exit 0, exercises the parents[1] loader).
"""
import os
import shutil
import subprocess
...
WRAPPER_SRC = REPO_ROOT / "quoin" / "scripts" / "affected_tests.py"
CORE_IMPL_SRC = REPO_ROOT / "quoin" / "core" / "scripts" / "affected_tests.py"
...
def test_installer_deployed_scripts_contains_affected_tests():
    """DEPLOYED_SCRIPTS in installer.py must contain 'affected_tests.py'.
    ...
    to ~/.claude/scripts/affected_tests.py (IVG-71 R-03).
    """
    ...
    assert "affected_tests.py" in mod.DEPLOYED_SCRIPTS, (
        "installer.py DEPLOYED_SCRIPTS must contain 'affected_tests.py'. "
        "Missing entry means install.sh won't deploy the adapter wrapper to "
        "~/.claude/scripts/affected_tests.py."
    )


def test_installer_core_scripts_contains_affected_tests():
    """CORE_SCRIPTS in installer.py must contain 'affected_tests.py'.
    ...
    fails at runtime with a FileNotFoundError (IVG-71 R-03).
    """
    import importlib.util
    ...
    assert "affected_tests.py" in mod.CORE_SCRIPTS, (
        "installer.py CORE_SCRIPTS must contain 'affected_tests.py'. "
        "Missing entry means install.sh won't deploy the core impl to "
        "~/.claude/core/scripts/affected_tests.py, causing the wrapper's "
...
@_dev_machine_only
def test_install_deploys_affected_tests_wrapper(tmp_path):
    """install.sh must deploy affected_tests.py to ~/.claude/scripts/."""
    ...
    deployed_wrapper = tmp_path / ".claude" / "scripts" / "affected_tests.py"
...
@_dev_machine_only
def test_install_deploys_affected_tests_core_impl(tmp_path):
    """install.sh must deploy affected_tests.py to ~/.claude/core/scripts/."""
    result = _run_install(tmp_path)
    assert result.returncode == 0, (
        f"install.sh failed: rc={result.returncode}\n"
        f"stdout: {result.stdout[:1500]}\nstderr: {result.stderr[:1500]}"
    ...
    deployed_core = tmp_path / ".claude" / "core" / "scripts" / "affected_tests.py"
