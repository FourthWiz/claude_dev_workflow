"""Phase 17 adapter pilot tests for weekly_review.

Parametrized over the single skill migrated in Phase 17. Follows the
pattern established by test_end_of_task_adapter_pilot.py,
test_run_adapter_pilot.py, and test_end_of_day_adapter_pilot.py.

Skill parameters: (skill_name, expected_model, has_section_0)
  - weekly_review: haiku, §0 present (cheap-tier — self-dispatches to Haiku)
"""
import re
...
def _slash_regex(name: str) -> re.Pattern:
    ...
    - NOT preceded by an alphanumeric char or [-_] (avoids matching path
      components like 'skills/weekly_review.md')
    - NOT followed by an alphanumeric char or [-_] (avoids matching
      'weekly_review-2026-05-11.md')
    """
    return re.compile(
        r"(?<![a-zA-Z0-9_\-])"
...
# ── Parametrized tests (run over the weekly_review skill) ────────────────────
...
def test_weekly_review_has_no_preamble_md():
    """weekly_review is NOT a subagent-preamble spawn target.
    ...
    weekly_review is excluded — `build_preambles.py` does not generate one.
    Asserting absence prevents accidental preamble leak.
    """
    legacy_preamble = PKG_DIR / "skills" / "weekly_review" / "preamble.md"
    adapter_preamble = (
        PKG_DIR / "adapters" / "claude" / "skills" / "weekly_review" / "preamble.md"
    )
    assert not legacy_preamble.exists(), (
        f"CONSTRAINT VIOLATION: {legacy_preamble} unexpectedly exists. "
        "weekly_review is NOT a spawn-target; build_preambles.py must not generate one."
    )
...
# ── Regression-guard tests (Phase 16 skills must still be covered) ────────


def test_phase_16_skills_still_covered():
    """Regression guard for the weekly_review work — verifies all 16 Phase 6–16 skills are still covered.

    Asserts that each Phase 6–16 migrated skill still has:
...
def test_install_fresh_clone_lists_weekly_review_in_migrated_skills():
    """Guard that test_install_fresh_clone.py includes weekly_review in MIGRATED_SKILLS (T-08 edit)."""
    ...
    assert '"weekly_review"' in tuple_region, (
        'test_install_fresh_clone.py MIGRATED_SKILLS tuple must include "weekly_review" '
        "(T-08 edit). The entry was reverted — restore it."
    )
