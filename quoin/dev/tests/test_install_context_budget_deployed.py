"""T-09 (IVG-141): install.sh deploys context_budget_guard.py + roster membership.
...
runs everywhere; the live install/deploy check is dev-machine-only (needs the
`claude` + `npx` binaries install.sh requires).
"""
import os
import re
import shutil
...
CORE_SRC = REPO_ROOT / "quoin" / "core" / "scripts" / "context_budget_guard.py"
WRAPPER_SRC = REPO_ROOT / "quoin" / "scripts" / "context_budget_guard.py"

SCRIPT = "context_budget_guard.py"


def _roster(name: str) -> list[str]:
    """Extract the entries of a tuple named `name` from installer.py by text.

    Line-based (comments contain stray `)`, so a non-greedy paren match would
    truncate the roster early). Reads from `NAME = (` to a line starting with `)`.
    """
    entries: list[str] = []
    collecting = False
...
def test_in_deployed_and_core_scripts():
    """Both rosters must list the wrapped script (import-free — runs on CI)."""
...
def test_install_deploys_context_budget_guard(tmp_path):
