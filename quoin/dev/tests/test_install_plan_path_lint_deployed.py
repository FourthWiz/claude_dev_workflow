"""IVG-143 T-05: install.sh deploys plan_path_lint.py to both targets.
...
1. Non-skipped unit-level assertion (runs on CI without `claude`):
   Import installer.py and assert "plan_path_lint.py" is in BOTH
...
WRAPPER_SRC = REPO_ROOT / "quoin" / "scripts" / "plan_path_lint.py"
CORE_IMPL_SRC = REPO_ROOT / "quoin" / "core" / "scripts" / "plan_path_lint.py"
...
def test_installer_deployed_scripts_contains_plan_path_lint():
    mod = _load_installer()
    assert "plan_path_lint.py" in mod.DEPLOYED_SCRIPTS, (
        "installer.py DEPLOYED_SCRIPTS must contain 'plan_path_lint.py'."
    )


def test_installer_core_scripts_contains_plan_path_lint():
    mod = _load_installer()
    assert "plan_path_lint.py" in mod.CORE_SCRIPTS, (
        "installer.py CORE_SCRIPTS must contain 'plan_path_lint.py' — the wrapper's "
...
def test_install_deploys_wrapper_and_core(tmp_path):
    ...
    wrapper = tmp_path / ".claude" / "scripts" / "plan_path_lint.py"
    core = tmp_path / ".claude" / "core" / "scripts" / "plan_path_lint.py"
