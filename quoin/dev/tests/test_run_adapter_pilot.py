"""Phase 15 adapter pilot tests for run.

Parametrized over the single skill migrated in Phase 15. Follows the
pattern established by test_end_of_task_adapter_pilot.py.

Skill parameters: (skill_name, expected_model, has_section_0)
  - run: opus, no §0 (Opus-tier orchestrator — mirrors architect, thorough_plan)
"""
import re
...
# Phase 6–14 skills — used for the regression-guard test below.
PRIOR_PHASE_SKILLS = [
...
# ── Parametrized tests (run over the run skill) ───────────────────────────
...
def test_run_has_no_preamble_md():
    """R-04 guard: run is NOT a subagent-preamble spawn target.
    ...
    run is excluded — `build_preambles.py` does not generate one.
    Asserting absence prevents accidental preamble leak.
    """
    legacy_preamble = PKG_DIR / "skills" / "run" / "preamble.md"
    adapter_preamble = PKG_DIR / "adapters" / "claude" / "skills" / "run" / "preamble.md"
    assert not legacy_preamble.exists(), (
        f"CONSTRAINT VIOLATION: {legacy_preamble} unexpectedly exists. "
        "run is NOT a spawn-target; build_preambles.py must not generate one."
    )
...
# ── Regression-guard tests (Phase 14 skills must still be covered) ────────


def test_phase_14_skills_still_covered():
    """Regression guard for the run work — verifies all 14 Phase 6–14 skills are still covered.

    Asserts that each Phase 6–14 migrated skill still has:
...
def test_install_fresh_clone_lists_run_in_migrated_skills():
    """Guard that test_install_fresh_clone.py includes run in MIGRATED_SKILLS (T-06 edit)."""
    ...
    assert '"run"' in tuple_region, (
        'test_install_fresh_clone.py MIGRATED_SKILLS tuple must include "run" '
