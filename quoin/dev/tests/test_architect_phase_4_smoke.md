...
```
grep -c '^### Phase 4:' quoin/skills/architect/SKILL.md
```
**Result:** 1 ✓
...
```
sed -n '/^### Phase 4:/,/^## Save session state/p' quoin/skills/architect/SKILL.md | grep -c 'model.*opus'
```
**Result:** 2 ✓
...
```
python3 -c "
with open('quoin/skills/architect/SKILL.md') as f: content = f.read()
...
## E. Recursive-self-critique guard: quoin prefixed form in fixture (expect ≥1)

```
grep -cE 'quoin/skills/(architect|critic)/SKILL\.md' <fixture>
```
**Result:** 2 ✓
...
```
sed -n '/^### Phase 4:/,/^## Save session state/p' quoin/skills/architect/SKILL.md | grep -c 'max_rounds'
```
**Result:** 12 ✓
...
```
sed -n '/^### Phase 4:/,/^## Save session state/p' quoin/skills/architect/SKILL.md | grep -cE 'PASS.*done|Max rounds reached|Loop detected'
```
**Result:** 3 ✓
...
```
grep -c 'no .workflow_artifacts/cache/ write-through' quoin/skills/architect/SKILL.md
```
**Result:** 1 ✓
...
```
grep -nE 'architect.*Phase 4' quoin/skills/critic/SKILL.md
```
...
```
grep -nE 'architecture-critic-\*' quoin/skills/critic/SKILL.md
```
**Result:** 2 hits (L14 and L51) ✓

## L. T-07: Phase 4 critic loop in quoin/CLAUDE.md (expect ≥1)

```
grep -n 'Phase 4 critic loop' quoin/CLAUDE.md
```
**Result:** 2 hits ✓
