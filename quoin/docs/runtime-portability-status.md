...
- Active Claude skills remain in `quoin/skills/`, EXCEPT the fourteen skills
  migrated across Phases 6–14 (`capture_insight`, `triage`, `start_of_day`,
  `review`, `plan`, `critic`, `revise`, `revise-fast`, `architect`,
  `thorough_plan`, `gate`, `implement`, `rollback`, `end_of_task`), which install from
...
- Portable intent docs for the migrated skills live at
  ...
  `quoin/core/skills/rollback.md`, and
...
- Skill bodies in `quoin/skills/` (stubs only for the 14 migrated skills; full bodies now in `quoin/adapters/claude/skills/<name>/SKILL.md`).
...
- Split shared skill intent from runtime overlays — partial: `capture_insight`, `triage`, `start_of_day`, `review`, `plan`, `critic`, `revise`, `revise-fast`, `architect`, `thorough_plan`, `gate`, `implement`, `rollback`, and `end_of_task` shipped under the adapter pattern across Phases 6–14.
- Execution-loop skill `run` and lifecycle/setup/support skills (`end_of_day`, `weekly_review`, `cost_snapshot`, `init_workflow`, `discover`, `expand`) remain future work; each warrants its own migration pass.
